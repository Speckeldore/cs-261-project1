...
class AVL(BST):
    ...
    def _balance_factor(self, node: AVLNode) -> int:
        ...
        TODO: Write your implementation
        """
        balancer = self._get_height(node.right) - self._get_height(node.left)
        node.balance = balancer
        return balancer
        "balanceFactor(N) = height(N.right) - height(N.left)"
        pass
    def _get_height(self, node: AVLNode, height = 0) -> int:
        """
        TODO: Write your implementation
        """
        maxi = 0
        if node.left is not None:
            ...
            if height >= maxi:
                maxi = height
                self.height = maxi
            self._get_height(node.left, height)
        if node.right is not None:
            height += 1
            if height >= maxi:
                maxi = height
            self.updateB(node.right, height)
        height += -1
        return maxi
        pass
    ...
    def _update_height(self, node: AVLNode) -> None:
        ...
        if node.parent == None:
            ...
            height = node.parent.height + 1
        node.height = height
        self.updateB(node,height)
    def updateB(self,node,height):
            if node.left is not None:
                height += 1
                self.updateB(node.left,height)
            node.height = height
            if node.right is not None:
                height += 1
                self.updateB(node.right,height)
    ...
    def _rebalance(self, node: AVLNode) -> None:
        ...
        "3 cases use node and child to determine if it RR LL RL LR"
